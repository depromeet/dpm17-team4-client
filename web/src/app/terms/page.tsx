...
const TermsPage = () => {
  ...
  const getPageTitle = () => {
    ...
    if (type === 'service') {
      const serviceTerm = filteredTerms.find(term => 
    ...
    if (type === 'privacy-policy') {
      const privacyTerm = filteredTerms.find(term => 
    ...
    switch (type) {
      case 'service':
        return terms.filter(term => 
          term.title.includes('서비스') || 
          term.title.includes('이용약관') );
      case 'privacy-policy':
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-[#121213] flex items-center justify-center">
        <div className="text-lg text-red-500">약관 정보를 불러오는데 실패했습니다.</div>
      </div>
    );
  }
