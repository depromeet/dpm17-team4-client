...
export const RecordDate = () => {
  ...
  // 날짜 변경 함수
  const updateDate = useCallback((newYear: number, newMonth: number, newDate: number) => {
  ...
  const handlePreviousDay = () => {
    const currentDate = new Date(parseInt(year), parseInt(month) - 1, parseInt(date));
    currentDate.setDate(currentDate.getDate() - 1);
    updateDate(currentDate.getFullYear(), currentDate.getMonth() + 1, currentDate.getDate());
  };
  ...
  return (
    <div className="px-[4.78rem] py-[1.25rem] text-h3 text-white text-center">
      <div className="flex items-center justify-between">
        <button onClick={handlePreviousDay} className="hover:opacity-70 transition-opacity">
          <ChevronLeft />
        </button>
        <h1>{getDateDisplayText()}</h1>
        <button onClick={handleNextDay} className="hover:opacity-70 transition-opacity">
          <ChevronRight />
        </button>
      </div>
    </div>
