...
async function handleMockFlow(data: SessionData) {
  ...
  session.accessTokenExpiresAt =
    Date.now() + AUTH_CONSTANTS.ACCESS_TOKEN_LIFESPAN * 1000;
...
async function handleRedirectFlow(
  ...
  const minimalUser = (({ id, nickname, avatarUrl }) => ({
    id,
    nickname,
    avatarUrl,
  }))(data.user);
  redirectResponse.cookies.set('temp_user', JSON.stringify(minimalUser), {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
    maxAge: AUTH_CONSTANTS.TEMP_COOKIE_MAX_AGE,
    path: '/',
  });
