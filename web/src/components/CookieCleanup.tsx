...
export default function CookieCleanup() {
  useEffect(() => {
    ...
    console.log(
      '📌 refreshToken이 document.cookie에 보이나요?',
      hasRefreshToken
    );

    if (!hasRefreshToken) {
      console.warn(
        '⚠️ refreshToken이 document.cookie에 없습니다. httpOnly=true로 설정되어 있을 수 있습니다!'
      );
      console.log(
        '💡 Application 탭 → Cookies에서 refreshToken의 HttpOnly 속성을 확인하세요.'
      );
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (stillHasToken) {
          console.log(
            '✅ refreshToken이 여전히 존재합니다 (정상 - kkruk.com 쿠키만 남음)'
          );
        } else {
