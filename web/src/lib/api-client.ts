...
apiClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (error.response?.status === 401) {
      // WebView 환경이면 refresh token으로 토큰 갱신 시도
      if (isWebViewAvailable()) {
        console.log('🔒 401 Unauthorized (WebView) - 토큰 갱신 시도');
        ...
        // Web 환경이면 refresh token이 없으므로 바로 로그아웃 후 재로그인
        console.log('🔒 401 Unauthorized (Web) - 로그아웃 후 재로그인');
        await logout();
        if (typeof window !== 'undefined') {
          window.location.href = '/auth';
        }
